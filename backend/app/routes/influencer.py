...
from ..services.worker_tasks import run_influencer_job
...
async def create_influencer_video(
    ...
    enforce_generation_capacity("influencer", user.user_id)
    reserve_generation_slot("influencer", user.user_id)
    ...
    try:
        ...
        try:
            ...
            try:
                credit_balance(user.user_id, billed_amount, "influencer_refund", f"Reembolso por cola no disponible {influencer_id}")
            except Exception:
                pass
            update_anim(
                influencer_id,
        ...
        return {"influencer_id": influencer_id, "status": "queued", "queue_task_id": queue_task_id}
    except Exception:
        release_generation_slot("influencer", user.user_id)
        raise
...
def get_influencer_video(influencer_id: str):
    ...
    if not video_path or not Path(video_path).exists():
        remote_url = str(item.get("video_storage_url") or "").strip()
        if not remote_url and item.get("video_storage_key"):
            from ..services.storage import resolve_download_url
            remote_url = resolve_download_url(str(item.get("video_storage_key") or ""), "")
        if remote_url:
